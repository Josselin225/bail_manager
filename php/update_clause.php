...
try {
    $stmt = $pdo->prepare(
        "UPDATE clauses_contrat SET titre = ?, contenu = ?, ordre_affichage = ?, actif = ? WHERE id = ?"
    );
    $stmt->execute([$titre, $contenu, $ordre_affichage, $actif, $id]);
    insertLog($pdo, "Clause contrat", "Modification de la clause #$id : $titre");
    flash('success', "Clause mise à jour avec succès.");
} catch (Exception $e) {
    error_log("Erreur modification clause : " . $e->getMessage());
